...
## Promoção de status
Uma implementação só pode passar de `experimental` para `stable` quando todos os cenários aplicáveis tiverem evidência registrada, não houver falha crítica de papel/nome/página atual/foco/ativação, e validação manual com recursos de tecnologia assistiva relevantess tiver ocorrido de fato.
