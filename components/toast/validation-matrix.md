...
## Promoção de status
Uma implementação só pode passar de `experimental` para `stable` quando todos os cenários aplicáveis tiverem evidência registrada, não houver falha crítica de anúncio/foco/temporização/ação, e validação manual com recursos de tecnologia assistiva relevantes tiver ocorrido de fato.
