...
## Promoção de status
Uma implementação só pode passar de `experimental` para `stable` quando todos os cenários aplicáveis tiverem evidência registrada, não houver falha crítica de nome/foco/acesso equivalente/conteúdo essencial e validação manual com tecnologia assistiva relevante tiver ocorrido de fato.
