...
Executar no Windows com leitor de tela suportado. Verificar conteúdo persistente, leitura por navegação, ausência de mudança de foco e integração do callback `on_announce` com a estratégia real do aplicativo. O callback isolado não constitui evidência de anúncio por recursos de tecnologia assistiva.
...
5. validação manual com recursos de tecnologia assistiva relevantesss tiver ocorrido de fato.
