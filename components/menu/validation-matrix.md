...
## Promoção de status
Uma implementação só pode passar de `experimental` para `stable` quando todos os cenários aplicáveis tiverem evidência registrada, não houver falha crítica de nome/papel/foco/ativação/fechamento e validação manual com recursos de tecnologia assistiva relevante tiver ocorrido de fato.
