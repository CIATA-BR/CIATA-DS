...
## Promoção de status
Uma implementação só pode passar de `experimental` para `estável` quando todos os cenários aplicáveis tiverem evidência registrada, não houver falha crítica de nome/foco/envio/feedback/privacidade/segurança e validação manual com recursos de tecnologia assistiva relevantes tiver ocorrido de fato.
